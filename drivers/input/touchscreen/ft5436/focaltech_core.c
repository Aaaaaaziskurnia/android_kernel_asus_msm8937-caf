...
int tpd_usb_plugin(int charger_on)
{
	FG_charging_status = charger_on;
	FTS_INFO("[fts] %s, charger plug %s", __func__, (charger_on == 1)?"IN":"OUT");
	return 0;
}
...
#if FTS_CHARGER_EN
	if((FG_charging_status) && (charging_flag == 0)) {
		charging_flag = 1;
		fts_enter_charger_mode(fts_i2c_client, true);
	}else if((FG_charging_status == 0) && (charging_flag == 1)) {
		charging_flag = 0;
...
static ssize_t switch_fw_version_read(struct switch_dev *sdev, char *buf)
{
    ...
    return sprintf(buf, "0x%x", uc_tp_fm_ver);
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = request_irq(client->irq, fts_ts_interrupt, IRQF_TRIGGER_FALLING, client->dev.driver->name, NULL);
